...
2. Resolve the global GitVaulty master identity. If it is missing interactively, offer to restore an existing
...
3. If `.gitvaulty/recipients.json` is missing, obtain a normalized username and initialize the
   owner, manager-signed default `team` group, repository configuration, and SOPS configuration without a
   permission prompt.
...
- **Restore existing backup** asks for the private `GITVAULTY-IDENTITY-...` value through the existing
...
- Missing identity or username in a non-interactive session: fail without partial identity input or
  secret output.
- Missing registry: create a fresh version 4 registry with the owner's public age recipient and
  Ed25519 signing key, a signed default-group policy, and related metadata.
