...
import { type Registry, type VaultUser, readRegistry, recipientsFor, writeRegistry } from "./registry.js";
...
export async function initialize(repo: Repository, user: { id: string; recipient: string }): Promise<void> {
...
export async function createVault(repo: Repository, name: string): Promise<void> {
  ...
  const recipient = await currentRecipient(repo);
  const user = registry.users.find((candidate) => candidate.recipient === recipient);
  if (!user) throw new GitVaultyError("Your local key is not registered. Ask an existing user to add its public recipient.");
...
export async function addUser(repo: Repository, user: VaultUser): Promise<void> {
  await ensureInitialized(repo);
  const registry = await readRegistry(repo);
  if (registry.users.some((item) => item.id === user.id || item.recipient === user.recipient)) throw new GitVaultyError("That user ID or recipient already exists.");
  ...
  await writeRegistry(repo, registry);
  try { for (const vault of user.vaults) await updateVaultKeys(repo, vaultFile(repo, vault)); }
  catch (error) {
    for (const [vault, contents] of snapshots) await writeFile(vaultFile(repo, vault), contents);
    registry.users = registry.users.filter((item) => item !== user); await writeRegistry(repo, registry); throw error;
  }
}

export async function removeUser(repo: Repository, id: string): Promise<void> {
  await ensureInitialized(repo);
  const registry = await readRegistry(repo);
  const user = registry.users.find((item) => item.id === id);
  if (!user) throw new GitVaultyError(`Unknown user: ${id}`);
  ...
  const snapshots = new Map<string, string>();
  for (const vault of user.vaults) snapshots.set(vault, await readFile(vaultFile(repo, vault), "utf8"));
  registry.users = registry.users.filter((item) => item.id !== id);
  await writeRegistry(repo, registry);
