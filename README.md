...
create a new key. It then creates a `team` group containing you, the public recipient registry,
repository preferences, SOPS configuration, and managed repository agent skill before continuing
the requested command. New and imported files use `team` by default, so the normal workflow needs no
access flags.
...
The repository skill maintained by GitVaulty teaches coding agents this workflow and warns them not to
