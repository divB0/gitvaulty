...
import { RUNTIME_VERSION, createRuntimeManifest, runtimeTarget } from "../scripts/package-tools.mjs";
...
describe("runtime packaging", () => {
  ...
  it("creates a deterministic exact release manifest", async () => {
    ...
    for (const target of targets) {
      await writeFile(path.join(directory, `gitvaulty-editor-runtime-v${RUNTIME_VERSION}-${target}.zip`), Buffer.from(target));
    }

    const manifest = await createRuntimeManifest(directory, `v${RUNTIME_VERSION}`, "https://github.com/divB0/gitvaulty");
    expect(manifest).toMatchObject({ protocolVersion: 1, runtimeVersion: RUNTIME_VERSION });
    expect(manifest.assets.map((asset: { target: string }) => asset.target)).toEqual(targets);
    expect(manifest.assets[0]).toMatchObject({
      target: "darwin-arm64",
      filename: `gitvaulty-editor-runtime-v${RUNTIME_VERSION}-darwin-arm64.zip`,
      size: Buffer.byteLength("darwin-arm64"),
      url: `https://github.com/divB0/gitvaulty/releases/download/v${RUNTIME_VERSION}/gitvaulty-editor-runtime-v${RUNTIME_VERSION}-darwin-arm64.zip`,
    });
